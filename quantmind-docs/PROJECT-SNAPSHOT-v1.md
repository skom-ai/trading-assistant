...
QuantMind is an **educational stock-opportunity decision-support tool** (NOT a broker, NOT
...
Next.js/Vite UI ──► NestJS BFF (P1 mediator, only public API) ──► FastAPI agent+engine ──► Postgres + Redis + Mongo
   quantmind-ui        quantmind-api/bff-nestjs                     quantmind-api/agent-fastapi   quantmind-db
```
...
- **Live Google Gemini** by default (`QM_LLM_PROVIDER=gemini`, `QM_LLM_MODEL=gemini-3.6-flash`) via
  the `google-genai` SDK, keyed by `GEMINI_API_KEY` (or `QM_LLM_API_KEY`). **IMPORTANT model note:**
  ...
  automatic fallback when no key is set (`QM_LLM_PROVIDER=mock`).
...
| UI unit (vitest) | `quantmind-ui/src/**.test.tsx` | 10 | 100% (tested scope) |
| UI e2e (playwright) | `quantmind-ui/e2e` | 4 | n/a |
| API integration (newman) | `quantmind-api/tests/postman` | 20 assertions | n/a |
| Security / PEN (pytest) | `quantmind-api/tests/pentest` | 9 | n/a |

Run: see `quantmind-api/tests/README.md`. Coverage exceeds the 95% target on the units that carry
...
├── quantmind-db/             # Postgres migrations V001-V006 + seeds S001/S002, Mongo init, Redis conf, Dockerfiles
├── quantmind-api/
...
├── quantmind-ui/             # existing Vite React UI (+ new Dockerfile); still mock-data driven
...
# Agent tests (Python 3.12):  cd quantmind-api/agent-fastapi && pip install -e ".[dev]" && pytest   (25 tests, ~93%)
# BFF tests (Node):           cd quantmind-api/bff-nestjs && npm install && npm test              (3 tests)
...
- **UI → BFF wiring:** a typed client exists (`quantmind-ui/src/api/client.ts`) and is unit-tested,
  but the screen components still render `src/data/mockData.ts` — swap each screen's data source to
  `quantmindApi.*` calls to go fully live in the browser.
