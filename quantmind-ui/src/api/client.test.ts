...
import { quantmindApi, type ApiError } from './client';
...
describe('quantmindApi', () => {
  ...
  it('newsCheck posts symbol + simulateEmpty', async () => {
    ...
    const out = await quantmindApi.newsCheck('NVDA', true);
  ...
  it('strategy posts symbol + newsLabel', async () => {
    ...
    const out = await quantmindApi.strategy('NVDA', 'REAL_CATALYST');
  ...
  it('throws the taxonomy envelope on a non-2xx response', async () => {
    ...
    await expect(quantmindApi.newsCheck('bad!')).rejects.toMatchObject<Partial<ApiError>>({
