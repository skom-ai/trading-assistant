...
Default `VT_LLM_PROVIDER=mock` — deterministic, evidence-driven, offline-testable. Set
`VT_LLM_PROVIDER=openrouter` + `VT_LLM_API_KEY=...` to swap in a live provider (no code change).
...
## Environment (prefix `VT_`)

`VT_POSTGRES_DSN`, `VT_REDIS_URL`, `VT_MONGO_URL`, `VT_LLM_PROVIDER`, `VT_LLM_API_KEY`,
`VT_LLM_MODEL`, `VT_INTERNAL_API_TOKEN`, `VT_SCANNER_WEIGHT_VERSION`, `VT_ANALOG_SIMILARITY_VERSION`.
